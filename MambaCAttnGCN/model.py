...
class Mamba_Layer(nn.Module):
    def __init__(self, mamba, d_model, dropout):
...
class DeepLncLoc(nn.Module):
    def __init__(self, w2v_emb, dropout, merge_win_size, context_size_list, out_size, d_state, d_conv):
        super().__init__()
        ...
        self.Textcnn_mamba_1 = Mamba_Layer(Mamba(w2v_emb.shape[1], d_state=d_state, d_conv=d_conv, expand=1), w2v_emb.shape[1], dropout)
        self.Textcnn_mamba_2 = Mamba_Layer(Mamba(w2v_emb.shape[1], d_state=d_state, d_conv=d_conv, expand=1), w2v_emb.shape[1], dropout)
        self.Textcnn_mamba_3 = Mamba_Layer(Mamba(w2v_emb.shape[1], d_state=d_state, d_conv=d_conv, expand=1), w2v_emb.shape[1], dropout)
        self.conv_output_1 = nn.Sequential(
                    nn.Conv1d(
                        in_channels=w2v_emb.shape[1],
                        out_channels=filter_out_size,
                        kernel_size=context_size_list[0],
                    ),
                    nn.ReLU(),
                    nn.AdaptiveMaxPool1d(1),
                    
                )
        self.conv_output_2 = nn.Sequential(
                    nn.Conv1d(
                        in_channels=w2v_emb.shape[1],
                        out_channels=filter_out_size,
                        kernel_size=context_size_list[1],
                    ),
                    nn.ReLU(),
                    nn.AdaptiveMaxPool1d(1),
                    
                )
        self.conv_output_3 = nn.Sequential(
                    nn.Conv1d(
                        in_channels=w2v_emb.shape[1],
                        out_channels=filter_out_size,
                        kernel_size=context_size_list[2],
                    ),
                    nn.ReLU(),
                    nn.AdaptiveMaxPool1d(1),
    ...
    def forward(self, p_kmers_id):
        ...
        #mamba path_1
        x_1 = self.Textcnn_mamba_1(x)
        x_1 = x_1.transpose(1, 2)
        x_1 = self.conv_output_1(x_1).squeeze(dim=2)
        #mamba path_2
        x_2 = self.Textcnn_mamba_2(x)
        x_2 = x_2.transpose(1, 2)
        x_2 = self.conv_output_2(x_2).squeeze(dim=2)
        #mamba path_3
        x_3 = self.Textcnn_mamba_3(x)
        x_3 = x_3.transpose(1, 2)
        x_3 = self.conv_output_3(x_3).squeeze(dim=2)
        #output
        x = torch.cat([x_1, x_3, x_2], dim=1)
        return x
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.addnorm = AddNorm([q_in_dim], dropout)
    def forward(self, queries, keys, values):
        
        Y = self.addnorm(queries, self.attention(queries, keys, values))
        return Y
